...
async function runtime(config = {}) {
    ...
    if (command === 'debug') {
        if (isNaN(parseInt(pinLength, 10))) {
            throw new Error('Invalid pin length given!');
        }

        const board = await createBoard();
        const strip = await createStrip(board, { length: parseInt(pinLength, 10) });
        strip.color('yellow');
        strip.show();
    ...
    const strip = await createStrip(board, { length: state.MAX_Y_INDEX + 1 });
